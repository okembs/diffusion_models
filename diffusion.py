...
class LatentDiffusionWrapper(nn.Module) : 
    def __init__(self , unetModel:Unet , autoencoder:VaeEncoder  ,autodecoder:VaeDecoder, clip_embedder:CLIP , latent_scaling_factor:float , n_steps:int , linear_start:float , linear_end:float) : 
        super().__init__()
        self.model = DiffusionModelWrapper(unetModel)
        self.first_stage = autoencoder
        self.second_stage = autodecoder
...
#define the beta schedule
        beta = torch.linspace(linear_end ** 0.5 , linear_end ** 0.5 , n_steps , dtype=torch.float) ** 2 
 ...
 #multiply the latentscalling facror by the autoencoder
...
Okembsmodel = LatentDiffusionWrapper( unetModel=Unet  , autodecoder=VaeDecoder , autoencoder=VaeEncoder , clip_embedder=CLIP  , latent_scaling_factor=2 , n_steps=10 , linear_start=1e-2 , linear_end=2e-1)
